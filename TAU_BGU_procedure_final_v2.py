...
# ------------------------------------SOURCE_METER_AND_AMPERMETER_CONFIGURATION----------------------------
source_meter = Keithley2450("GPIB::11")  # the GPIB address needs to be checked in the ICS GPIB Explorer
source_meter.source_mode = 'voltage'          # sets the Keithley 2450 to act as a voltage source

# ------------------------------------STEPPER_MOTOR_CONFIGURATION------------------------------------------
stepper = SchniderLMDCM572("COM4", 9600)  # check COM channel number  n the device manager window

# ------------------------------------APPROACH_FUNCTION_CONFIGURATION--------------------------------------

def approach(supply_voltage, th_current, approach_speed, direction):
    """inputs explanation
    thCurrent = threshold current (e.g 3e-6, 8e-6)
    approachSpeed = steppers slew speed, this is an integer
    direction = approach direction (-1 for approaching and 1 for retracting)"""

    valid_directions = {1, -1}
    if direction not in valid_directions:
        raise Exception("direction value must be 1 or -1")
    if th_current == 0:
        raise Exception("Threshold current must be larger than 0")

    source_meter.source_voltage = supply_voltage    # voltage supply value
    source_meter.enable_source()    # turns on the supply
    approach_speed = approach_speed * direction  # sets the motors slew speed with the appropriate direction
    stepper.move(approach_speed)  # move the motor with the desired direction and slew speed
    current_sample = source_meter.current  # sample the current
    current_sample = float(current_sample[:13])
    while current_sample <= th_current:  # while the measured current is below the desired threshold
        if keyboard.is_pressed("\u001b"):  # press 'esc' for killing the approach
            stepper.move(0)
            break
        current_sample = source_meter.current  # current sample executed by the Keithley2450
        current_sample = float(current_sample[:13])
        print(current_sample)  # prints the measured current
    stepper.move(0)  # stop the motor when reaching the desired threshold current

# ------------------------------------CONSOLE_CONTROL_CONFIGURATION--------------------------------------

while True:
    query = input("begin approach? (y/n): ")
    if query == "n":  # query for beginning the approach
        break
    elif query == "y":
        thCurrent = float(input("Enter threshold current: "))  # for controlling the approach via console
        supplyVoltage = float(input("Enter supply voltage: "))
        approachSpeed = int(input("Enter approach speed: "))
        approachDirection = int(input("Enter approach direction: "))
        approach(supplyVoltage, thCurrent, approachSpeed, approachDirection)
    else:
        raise Exception("user input must be 'y' or 'n'")

##stepper.move(2500)                         # uncomment the two lines for retracting the Tiprobe (if necessary)
##time.delay(4)
